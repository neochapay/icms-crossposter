...
function blogs(){
	...
	$do 	   = cmsCore::request('do', 'str', 'latest');
...
//Вставка компонента кросспостинг
	$inCore->loadModel('crossposter');
	$cp_model = new cms_model_crossposter();
	$have_crosspost = $cp_model->getPoster($inUser->id);
//конец вставки компонента кросспостинг 
        //показываем форму
...
// Вставка компонента кросспостинг
		$smarty->assign('have_crosspost', $have_crosspost);
// Конец вставки компонента кросспостинг
		$smarty->assign('is_admin', $inUser->is_admin);
		$smarty->assign('myblog', $myblog);
...
//Вставка компонента кросспостинг
		      $crosspost_lj  = $inCore->request('crosspost_lj','str');
		      if($crosspost_lj == "on")
		      {
			$inCore->loadModel('crossposter');
			$cp_model = new cms_model_crossposter();
			$cp_model->addPost($added['id']);
		      }
// Конец вставки компонента кросспостинг
			if ($mod['published']) {
				...
				if ($blog['allow_who'] != 'nobody' && $mod['allow_who'] != 'nobody'){
					...
					cmsActions::log('add_post', array(
						'object' => $mod['title'],
						'object_url' => $model->getPostURL($blog['seolink'], $added['seolink']),
						'object_id' => $added['id'],
						'target' => $blog['title'],
